<div>
  ...
  <div class="p-8">
    ...
    <div class="flex flex-col">
      ...
      <div class="field">
        <select name="project_id" id="project_id" class="input --select" wire:model="project_id">
          <option value="">-</option>
          @foreach($selectProjects as $i => $n)
            <option value="{{ $i }}">{{ $n }}</option>
          @endforeach
        </select>
        <div class="reinit --select" wire:click="reinit('project_id')">
          <i aria-hidden="true" class="las la-backspace"></i>
        </div>
      </div>
    </div>
